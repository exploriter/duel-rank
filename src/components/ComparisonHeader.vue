    ...
    <div class="relative p-6">
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center space-x-4">
          <!-- List Icon -->
          <div
              class="w-14 h-14 bg-white/20 backdrop-blur-sm border border-white/30 rounded-lg flex items-center justify-center inset-ring inset-ring-violet-700/30">
          ...
          <div>
            ...
            <div class="text-sm font-medium text-white/80">
              {{ activeList?.items.length || 0 }} items • {{ getCompletedComparisons(activeList) }} comparisons •
              {{ formatDate(activeList?.createdAt) }}
            </div>
          </div>
        </div>
        ...
        <div v-if="lists.length > 1" class="flex items-center space-x-3">
          ...
          <div class="relative">
            ...
            >
              ...
              <svg class="w-4 h-4 text-white/70 group-hover:text-white transition-colors"
                   :class="{ 'rotate-180': showListSelector }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="p-2">
        ...
        <div class="space-y-1">
          ...
          >
            ...
            <span v-if="list.id === selectedListId"
                  class="ml-2 w-2 h-2 bg-indigo-500 rounded-full flex-shrink-0"></span>
          </button>
        </div>
      </div>
    </div>
