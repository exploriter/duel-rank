...
<template>
  <div class="bg-gradient-to-r from-gray-50 to-slate-50 border border-gray-200 rounded-xl p-6">
    ...
    <div v-if="hasWorkableRanking && !isComparisonComplete && !refining" class="mt-4">
      <div
          class="flex items-center justify-between p-4 bg-gradient-to-r from-violet-50 to-indigo-50 border border-indigo-200 rounded-xl">
        <div class="flex items-center space-x-2">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
               class="size-5 text-violet-800">
            <path
          ...
          <span
              class="text-sm font-medium text-violet-900">Initial ranking complete. Some positions are inferred.</span>
        </div>
        <button
    ...
    <div v-else-if="isComparisonComplete && remainingPairs === 0" class="mt-4">
      <div
          class="flex items-center space-x-2 p-4 bg-gradient-to-r from-violet-50 to-indigo-50 border border-indigo-200 rounded-xl">
